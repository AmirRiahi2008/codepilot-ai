...
export class InternalController {
  ...
  progress(
    @Param('id') id: string,
    @Body() body: { status: 'RUNNING' | 'DOWNLOADING' | 'SCANNING' | 'AI_ANALYSIS' },
    @Headers('x-worker-secret') secret?: string,
  ) {
    return this.service.progress(id, body.status, secret);
  }
  ...
  complete(
    ...
    return this.service.complete(id, payload, secret);
  }
  ...
  fail(
    ...
    return this.service.fail(id, body.reason, secret);
  }
}