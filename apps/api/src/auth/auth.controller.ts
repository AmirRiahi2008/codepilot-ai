...
export class AuthController {
  ...
  async register(@Body() body: AuthDto, @Res({ passthrough: true }) res: Response) {
    const { token } = await this.auth.register(body.name, body.email.toLowerCase(), body.password);
  ...
  async login(@Body() body: AuthDto, @Res({ passthrough: true }) res: Response) {
    const { token } = await this.auth.login(body.email.toLowerCase(), body.password);
    this.setCookie(res, token);
    return { ok: true };
  }
  ...
  @UseGuards(AuthGuard)
  @Get('me')
  me(@Req() req: Request & { userId: string }) { return this.auth.me(req.userId); }
